...
function search_all_profiles(){ //スペル修正 search に変更
    //db接続を確立
    ...
    //クエリとして用意
    $search_query = $search_db->prepare($search_sql); //スペル修正 search に変更

    //SQLを実行
    try{
        $search_query->execute(); //スペル修正 search に変更
    } catch (PDOException $e) {
        $search_query=null;       //スペル修正 search に変更
        return $e->getMessage();
    }

    //全レコードを連想配列として返却
    return $search_query->fetchAll(PDO::FETCH_ASSOC);  ////スペル修正 search に変更
}
 ...
 * @return type
 */
function search_profiles($name=null,$year=null,$type=null){  //スペル修正 aを追加
    //db接続を確立
    $search_db = connect2MySQL();
    ...
    //クエリとして用意
    $search_query = $search_db->prepare($search_sql); //スペル修正 seatch -> search
    if(isset ($name)){
...
function delete_profile($id){
    ...
    $delete_sql = "DELETE  FROM user_t WHERE userID=:id";   // 小文字のtをTに変更
...
function update_profile($name,$birthday,$tell,$type,$comment,$id){ //追加修正 関数を完成
    //db接続を確率
