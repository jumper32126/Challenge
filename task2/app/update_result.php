...
<meta charset="UTF-8">
  ...
  <body><?php
    ...
    if(!isset ($_POST['mode']) || !$_POST['mode']=="RESULT"){ //追加修正 直接アクセス禁止
  ...
  ?>
    ...
    $result = update_profile($name,$birthday,$tell,$type,$comment,$id);//追加修正 引数を追加
    //エラーが発生しなければ表示を行う
    if(!isset($result)){
    ?>
    <h1>更新確認</h1>
    ...
    名前:<?php echo $name.'<br>';?>     <? // 追加修正 変更データを表示?>
    誕生日:<?php echo $birthday.'<br>';?>
    電話番号:<?php echo $tell.'<br>';?>
    種別:<?php echo ex_typenum($type).'<br>';?>
    自己紹介文:<?php echo $comment.'<br>';?>
